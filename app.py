...
googleSheetId1 = '115_XlSdHIQwPZ_W4j_JD-jVhs_8NHGHkHmxTj5il9dY'
worksheetName1 = 'sheet1'
URL1 = 'https://docs.google.com/spreadsheets/d/{0}/gviz/tq?tqx=out:csv&sheet={1}'.format(
googleSheetId1,
worksheetName1
)
df = pd.read_csv(URL1, keep_default_na=False)
df3=df.to_dict(orient='list')
d={}
for i in df3:
    test_list = [j for j in df3[i] if j]
    d[i]=test_list
